...
function buildTal(data){
	...
	for(var o in data.info){
		...
		$(resub1).find("input[name='reSub1']").val(data.info[o].stuInterview);
		var resub2= $('<td><input type="text" name="reSub2"></td>');
		$(resub2).find("input[name='reSub2']").val(data.info[o].stuReenglish);
...
$(document).on("click",".btn1",function(){
	...
	$.ajax({
		url:"${path}/addReTestScore2?stuId="+id+"&&stuReenglish="+resub2+"&&stuInterview="+resub1,
		type:"GET",
		success:function(data){
			if(data.msg=="success"){
				var type = "${type}";
